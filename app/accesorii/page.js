  ...
  useEffect(() => {
    ...
    const ProductEngine = {
      ...
      buildCard(produs) {
        ...
        const typeAttr = (produs.tip_accesoriu || '').toLowerCase();
        const connAttr = (produs.tip_conector || '').toLowerCase();
        const conditionAttr = produs.stare || 'SH';
        const stocDisponibil = produs.stoc || 0;
        const linkProdus = `/accesorii/produs?id=${produs.id}`;
        const bgText = produs.brand ? produs.brand.toUpperCase() : 'ACCESORIU';
        let displayVariant = typeAttr ? typeAttr.replace('-', ' ').toUpperCase() : 'ACCESORIU';
        if (connAttr) displayVariant += ` • ${connAttr.toUpperCase()}`;
        ...
        card.setAttribute('data-connector', connAttr);
        ...
        card.innerHTML = `
          <div class="bg-text" style="font-size: 40px;">${bgText}</div>
      ...
      getActiveFilters() {
        const filters = { brand: [], type: [], connector: [] };
      ...
      applyFilters(products) {
        ...
        return products.filter(p => {
          ...
          const type = (p.tip_accesoriu || '').toLowerCase();
          const conn = (p.tip_conector || '').toLowerCase();
          if (f.brand.length && !f.brand.includes(brand)) return false;
          if (f.type.length && !f.type.includes(type)) return false;
          if (f.connector.length && !f.connector.includes(conn)) return false;
          return true;
        });
      },
      ...
      renderActiveTags() {
        ...
        const f = this.getActiveFilters();
        const allActive = [...f.brand, ...f.type, ...f.connector];
      ...
      <header className="category-hero-ultra">
        <div className="section-container">
          <div className="hero-content-wrapper" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <h1 className="page-title-ultra">Accesorii</h1>
          </div>
        </div>
      </header>
