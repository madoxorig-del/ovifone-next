...
export default function Folii() {
  ...
  useEffect(() => {
    ...
    const ProductEngine = {
      ...
      async fetchProducts() {
        try {
          const { data: produse, error } = await supabase.from('produse').select('*').eq('categorie', 'folii').order('created_at', { ascending: false });
      ...
      buildCard(produs) {
        ...
        const linkProdus = `/folii/produs?id=${produs.id}`;
        const bgText = produs.brand ? produs.brand.toUpperCase() : 'FOLIE';
      ...
      <header className="category-hero-ultra">
        <div className="section-container">
          <div className="hero-content-wrapper" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <h1 className="page-title-ultra">Folii Protecție Ecran</h1>
          </div>
        </div>
      </header>
