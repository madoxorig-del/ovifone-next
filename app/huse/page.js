...
export default function Huse() {
  ...
  useEffect(() => {
    ...
    const ProductEngine = {
      ...
      async fetchProducts() {
        try {
          const { data: produse, error } = await supabase.from('produse').select('*').eq('categorie', 'huse').order('created_at', { ascending: false });
      ...
      buildCard(produs) {
        ...
        const compatAttr = (produs.compatibilitate || produs.brand || '').toLowerCase();
        const materialAttr = (produs.material || '').toLowerCase();
        const featuresAttr = `${compatAttr} ${materialAttr} ${produs.nume ? produs.nume.toLowerCase() : ''}`;
        const conditionAttr = produs.stare || 'SH';
        const stocDisponibil = produs.stoc || 0;
        const linkProdus = `/huse/produs?id=${produs.id}`;
        const bgText = produs.brand ? produs.brand.toUpperCase() : 'HUSĂ';
        let displayVariant = compatAttr ? `Pt. ${compatAttr.charAt(0).toUpperCase() + compatAttr.slice(1)}` : '';
        if (materialAttr) displayVariant += ` • ${materialAttr}`;

        const card = document.createElement('div');
        card.className = 'dark-product-card product-item';
        card.setAttribute('data-brand', brandAttr);
        card.setAttribute('data-compat', compatAttr);
        card.setAttribute('data-material', materialAttr);
        ...
        card.innerHTML = `
          ...
          <div class="img-container"><a href="${linkProdus}" style="display:contents;"><img src="${produs.imagine_url}" alt="${produs.nume}" decoding="async" loading="lazy"></a></div>
          <div class="container-info">
            <h2>${produs.nume}</h2>
            <p class="variant-text">${displayVariant || 'Husă protecție'}</p>
            ...
            <div class="cartBtn-wrap">
              <button class="cartBtn ${stocDisponibil <= 0 ? 'disabled' : ''}" ${stocDisponibil <= 0 ? 'style="background-color:#ccc;cursor:not-allowed;color:#666;"' : ''} data-nume="${produs.nume}" data-pret="${produs.pret}" data-img="${produs.imagine_url}" data-variant="${displayVariant || 'Standard'}" data-stoc="${stocDisponibil}">
      ...
      getActiveFilters() {
        const filters = { compat: [], material: [], features: [] };
      ...
      applyFilters(products) {
        ...
        return products.filter(p => {
          ...
          if (f.features.length && !f.features.some(v => features.includes(v))) return false;
          return true;
        });
      },
      ...
      renderActiveTags() {
        ...
        const f = this.getActiveFilters();
        const allActive = [...f.compat, ...f.material, ...f.features];
      ...
      clearAllActiveFilters() { DOM.filterCheckboxes.forEach(cb => { cb.checked = false; }); if (DOM.pageTitle) DOM.pageTitle.textContent = 'Huse'; this.executeFilteringAndSorting(); }
    };
      ...
      <header className="category-hero-ultra">
        <div className="section-container">
          <div className="hero-content-wrapper" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <h1 className="page-title-ultra">Huse</h1>
          </div>
        </div>
      </header>
            ...
            <div className="filter-scroll-area">
              ...
              <div className="filter-widget-ultra">
                ...
                <div className="filter-options-ultra" data-filter-group="material">
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="silicon" /><span className="box-ultra"></span> <span className="lbl-text">Silicon / Gel TPU</span></label>
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="piele" /><span className="box-ultra"></span> <span className="lbl-text">Piele Naturală / Eco</span></label>
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="kevlar" /><span className="box-ultra"></span> <span className="lbl-text">Kevlar / Fibră de Carbon</span></label>
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="plastic" /><span className="box-ultra"></span> <span className="lbl-text">Plastic Dur / PC</span></label>
                </div>
              </div>
              <hr className="filter-divider" />
              <div className="filter-widget-ultra">
                <h4 className="widget-title-ultra">Caracteristici Speciale</h4>
                <div className="filter-options-ultra" data-filter-group="features">
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="magsafe" /><span className="box-ultra"></span> <span className="lbl-text">Compatibil MagSafe</span></label>
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="transparenta" /><span className="box-ultra"></span> <span className="lbl-text">Transparentă (Clear)</span></label>
                  <label className="cyber-checkbox-ultra"><input type="checkbox" value="antisoc" /><span className="box-ultra"></span> <span className="lbl-text">Anti-șoc (Armură)</span></label>
                </div>
              </div>
            </div>
          </aside>
